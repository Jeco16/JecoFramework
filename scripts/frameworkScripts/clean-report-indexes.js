...
if (fs.existsSync(rootIndex)) {
  try { fs.unlinkSync(rootIndex); console.log('Removed playwright-report/index.html'); }
  catch (e) { console.warn('Failed removing root index.html', e); }
}

if (explicitSuite) {
  const idx = path.join(root, explicitSuite, 'index.html');
  if (fs.existsSync(idx)) {
    try { fs.unlinkSync(idx); console.log(`Removed ${path.join('playwright-report', explicitSuite, 'index.html')}`); }
    catch (e) { console.warn('Failed removing', idx, e); }
  } else {
...
for (const name of fs.readdirSync(root)) {
  ...
  try {
    ...
    if (fs.existsSync(idx)) {
      try { fs.unlinkSync(idx); console.log(`Removed ${path.join('playwright-report', name, 'index.html')}`); }
      catch (e) { console.warn('Failed removing', idx, e); }
    }
  } catch (e) { /* ignore */ }
}
