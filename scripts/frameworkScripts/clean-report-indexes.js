...
if (explicitSuite) {
  ...
  if (fs.existsSync(idx)) {
    try {
      fs.unlinkSync(idx);
      console.log(
        `Removed ${path.join("playwright-report", explicitSuite, "index.html")}`,
      );
    } catch (e) {
    ...
    console.log(
      `No existing index.html to remove in playwright-report/${explicitSuite}`,
    );
  }
  ...
  try {
    ...
    if (fs.existsSync(idx)) {
      try {
        fs.unlinkSync(idx);
        console.log(
          `Removed ${path.join("playwright-report", name, "index.html")}`,
        );
      } catch (e) {
        console.warn("Failed removing", idx, e);
      }
