...
//   by matching candidate suite names from tests/suites/*/suite.config.js inside the HTML.
...
function discoverCandidateSuites() {
  ...
  try {
    if (!fs.existsSync(testsSuites)) return candidates;
    for (const dir of fs.readdirSync(testsSuites)) {
      const cfg = path.join(testsSuites, dir, 'suite.config.js');
...
const runs = fs
  .readdirSync(root)
  .filter((n) => n.startsWith('run-') && fs.statSync(path.join(root, n)).isDirectory());
  ...
  for (const r of runs) {
    ...
    try {
      ...
      if (r.startsWith('run-')) {
        ...
        try {
          preserveAttachmentsAndRemove(targetRunPath, path.join(root, explicitSuite), r);
          console.log(
