...
const runs = fs.readdirSync(root).filter(n => n.startsWith('run-') && fs.statSync(path.join(root, n)).isDirectory());
...
function moveIndex(srcIndexPath, suiteName) {
  ...
  try { fs.renameSync(srcIndexPath, dst); }
  catch (e) { fs.copyFileSync(srcIndexPath, dst); fs.unlinkSync(srcIndexPath); }
}
...
if (explicitSuite) {
  ...
  if (fs.existsSync(rootIndex)) {
    try { moveIndex(rootIndex, explicitSuite); console.log(`Moved index.html into playwright-report/${explicitSuite}`); }
    catch (e) { console.error('Failed moving root index.html:', e); }
  }
    for (const r of runs) {
    ...
    catch (e) { console.warn('Error moving from', r, e); }
  }
  process.exit(0);
}
...
if (runs.length === 0) {
  ...
  if (fs.existsSync(rootIndex) && candidates.length === 1) {
    ...
    console.log(`Moved root index.html into playwright-report/${candidates[0]}`);
    process.exit(0);
  }
  ...
  if (existingSuites.length > 0) {
    console.log(`Report already present in suite folder(s): ${existingSuites.join(', ')}; nothing to move.`);
    process.exit(0);
  }

  console.log('No run-* folders found and no explicit suite provided — nothing to move.');
  process.exit(0);
}
