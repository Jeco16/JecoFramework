...
async function main() {
  ...
  try {
    ...
    console.log('Before targeted cleanup, playwright-report contains:', fs.existsSync(path.resolve(process.cwd(), 'playwright-report')) ? fs.readdirSync(path.resolve(process.cwd(), 'playwright-report')) : '<missing>');
    const cleaner = spawn('node', cleanArgs, { stdio: 'inherit', shell: true });
    await new Promise((res) => cleaner.on('exit', res));
    console.log('After targeted cleanup, playwright-report contains:', fs.existsSync(path.resolve(process.cwd(), 'playwright-report')) ? fs.readdirSync(path.resolve(process.cwd(), 'playwright-report')) : '<missing>');
  ...
  if (!opts.config && !opts.suite) {
    ...
    try {
      if (fs.existsSync(testsE2E)) {
        const subs = fs.readdirSync(testsE2E).filter(d => fs.statSync(path.join(testsE2E, d)).isDirectory());
        if (subs.length === 1) {
  ...
  if (opts.clean) {
    try {
      fs.rmSync(path.resolve(process.cwd(), 'playwright-report'), { recursive: true, force: true });
      fs.rmSync(path.resolve(process.cwd(), 'artifacts'), { recursive: true, force: true });
      console.log('Cleaned previous reports/artifacts');
    } catch (e) { /* ignore */ }
  }

  // prepare suite folder (removes only index.html)
  console.log('Before prepare-report, playwright-report contains:', fs.existsSync(path.resolve(process.cwd(), 'playwright-report')) ? fs.readdirSync(path.resolve(process.cwd(), 'playwright-report')) : '<missing>');
  const prep = spawn('node', ['./scripts/frameworkScripts/prepare-report.js', opts.suite], { stdio: 'inherit', shell: true });
  await new Promise((res) => prep.on('exit', res));
  console.log('After prepare-report, playwright-report contains:', fs.existsSync(path.resolve(process.cwd(), 'playwright-report')) ? fs.readdirSync(path.resolve(process.cwd(), 'playwright-report')) : '<missing>');
    ...
    if (fs.existsSync(reportRoot)) {
      ...
      for (const name of fs.readdirSync(reportRoot)) {
        ...
        try {
          ...
          console.log(`Backed up existing report folder ${name} -> .playwright-report-backup/${name}`);
  ...
  child.on('exit', (code) => {
    ...
    console.log('Before move-report, playwright-report contains:', fs.existsSync(path.resolve(process.cwd(), 'playwright-report')) ? fs.readdirSync(path.resolve(process.cwd(), 'playwright-report')) : '<missing>');
    const mover = spawn('node', ['./scripts/frameworkScripts/move-report.js', opts.suite], { stdio: 'inherit', shell: true });
    mover.on('exit', () => {
      console.log('After move-report, playwright-report contains:', fs.existsSync(path.resolve(process.cwd(), 'playwright-report')) ? fs.readdirSync(path.resolve(process.cwd(), 'playwright-report')) : '<missing>');
