...
async function main() {
  ...
  // If no args provided, and there's a single suite under tests/suites, use it
  if (!opts.config && !opts.suite) {
    const testsSuites = path.resolve(process.cwd(), 'tests', 'suites');
    try {
      if (fs.existsSync(testsSuites)) {
        const subs = fs
          .readdirSync(testsSuites)
          .filter((d) => fs.statSync(path.join(testsSuites, d)).isDirectory());
        if (subs.length === 1) {
          opts.config = path.join('tests', 'suites', subs[0], 'suite.config.js');
          opts.suite = subs[0];
  ...
  if (opts.suite && !opts.config) {
    const candidateCfg = path.join('tests', 'suites', opts.suite, 'suite.config.js');
  ...
  if (opts.suite) {
    const suitePath = `tests/suites/${opts.suite}`; // use forward slashes for CLI
    args.push(suitePath);
  }
  ...
  // Forward leftover CLI args (e.g. --grep) to Playwright
  const knownPrefixes = ['--config=', '--suite='];
  const knownFlags = ['--clean'];
  const forwardArgs = argv.filter(a => !knownPrefixes.some(p => a.startsWith(p)) && !knownFlags.includes(a));
  if (forwardArgs.length) args.push(...forwardArgs);
