...
async function main() {
  ...
  if (opts.config) {
    try {
      ...
      if (!parsedWithAST) {
        ...
        const startIdx =
          exportIdx >= 0 ? exportIdx : constIdx >= 0 ? constIdx : -1;
        if (startIdx >= 0) {
      ...
      if (!suiteEnv)
        suiteEnv = process.env.ENV || process.env.NODE_ENV || "dev";
  ...
  if (!opts.suite) {
    console.error(
      "Suite name not provided. Use --suite=<name> or export SUITE_NAME.",
    );
    process.exit(1);
  }
  ...
  const prep = spawn(
    "node",
    ["./scripts/frameworkScripts/prepare-report.js", opts.suite],
    {
      stdio: "inherit",
      shell: true,
    },
  ...
  child.on("exit", (code) => {
    ...
    const mover = spawn(
      "node",
      ["./scripts/frameworkScripts/move-report.js", opts.suite],
      {
        stdio: "inherit",
        shell: true,
      },
